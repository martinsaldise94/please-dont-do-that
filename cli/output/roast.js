...
const INTROS = [
  [
    80,
    "Congratulations. You've built the same website as 40,000 other people. This thing has more AI fingerprints than a ChatGPT terms-of-service page.",
  ],
  [
    60,
    'Oh wow, a hero section, some feature cards, and a gradient. Bold choices. Truly. Swap the logo and this belongs to your competitor.',
  ],
  [
    40,
    'Some personality in here, but also enough generic patterns that even the AI is embarrassed.',
  ],
...
export function formatRoast(result) {
  ...
  if (result.industry) {
    const { matchedTerms } = result.industrySignals;
    lines.push('');
    lines.push(
      matchedTerms.length === 0
        ? `Scored as ${result.industry}. Not one term a real ${result.industry} business would use. Nothing here says what you actually do.`
        : `Scored as ${result.industry}. ${matchedTerms.length} real term${matchedTerms.length === 1 ? '' : 's'} found: ${matchedTerms.slice(0, 4).join(', ')}.`,
    );
  }
  lines.push('');
  ...
  lines.push('');
  lines.push(
    `Engine v1 · Rules ${result.rulesVersion} · Lang: ${result.detectedLang} · pdtd ${result.pdtdVersion}`,
  );
  lines.push('');
